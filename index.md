...
- **[Comparison to other runtimes](COMPARISON.md)** — what a commercial platform, Apache Camel and
  Workato have published, under what conditions, and which of their scenarios have been
  rebuilt here
...
| 005 | [http-proxy](scenarios/005-http-proxy/) | Pass a request to a backend that takes 70 ms. Built to a commercial platform's published spec. |
| 006 | [json-transform](scenarios/006-json-transform/) | Reshape a JSON collection two ways. Mirrors a commercial platform's payload transformation case. |
...
load generator**. That is a recorded limitation, not a controlled variable. Cross-vendor
throughput comparisons are not defensible from this host; footprint and CPU-ms per request
are. See [COMPARISON.md](COMPARISON.md) for where that line is drawn and why.
